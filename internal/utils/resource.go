...
// for embedding into the Portal binary.
...
func WindowsResource(version, outputFile, tempDir string) error {
	...
	// StringFileInfo
	vi.StringFileInfo.CompanyName = "https://lumeweb.com"
	vi.StringFileInfo.ProductName = "Lume Web Portal"
	vi.StringFileInfo.FileDescription = "Lume Web Portal"
	vi.StringFileInfo.InternalName = strings.TrimSuffix(basename, ext)
	vi.StringFileInfo.OriginalFilename = basename
	vi.StringFileInfo.LegalCopyright = "Hammer Technologies LLC"
	...
	ico, err := embedFS.ReadFile("resources/ico/xportal.ico")
	if err != nil {
		return err
	}
	icoCopy, err := os.Create(filepath.Join(tempDir, "xportal.ico"))
	if err != nil {
		return err
	}
	// set ico path
